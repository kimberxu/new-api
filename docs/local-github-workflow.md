...
| `model/main.go` | PostgreSQL 连接强制 `PreferSimpleProtocol: true`（兼容 PgBouncer/Neon/Supabase）+ `normalizePostgresDSN` 兜底 `client_encoding=UTF8` | 上游新增 `ensureUserQuotaColumns`、`migratePrefillGroupUniqueness`（参数化 raw SQL）与 `migrateDBFast` 删除 | `3621f98f9`：2026-09-01 fix；上游 `migratePrefillGroupUniqueness` 的 `db.Raw(...to_regclass(?) ...)` 参数化查询在 personal `PreferSimpleProtocol` + PG 连接编码非 UTF8 环境触发 pgx `sanitizeForSimpleQuery` 强制 `client_encoding=UTF8`（conn.go:1265）→ 启动 FATAL；新增 `normalizePostgresDSN` 兜底给 DSN 追加 `client_encoding=UTF8` (runtime param)，已显式携带时尊重用户值不覆盖 |
