...
> 对应分支:`personal` 基线 `317e9ddd`(2026-08-29 刷新至 `668213d7`；`personal` 线同步流程见「同步上游」节)
...
| `controller/channel-test.go` | 模型级禁用/恢复分支、`ShouldDisableChannelWithDecision`、`processChannelError` 第 4 参 `nil` | 上游 `4add708e` 把单渠道测试重构成 `runChannelTestWorkers` worker 池，循环体整体搬家 | `235ae5a7`：2026-08-24 同步 6 提交；上游重构后的新调用点逐处补魔改行（判定换 `WithDecision`、模型级 if/else 包裹、enable 块后插模型级恢复块、`performChannelTests` 调用前插 `recoverExpiredModelBans` 且保留上游新增的 `concurrency` 参数）；`668213d7`：2026-08-29 同步 6 提交；上游 int32 重构移除 `math.Round`，魔改随机化提交新增 `math/rand/v2` 后 `math` 成孤儿，autosquash 修正 |
| `service/quota.go` | 计费功能级移除（`PreWssConsumeQuota` 等短路、`strings` import 随之删除） | 上游 `a073f74b` int32 重构把 `math.Round` 换成 `common.QuotaRoundChecked` 并调整 import | `668213d7`：2026-08-29 同步 6 提交；import 冲突净结果为两侧删除（`math` 被上游移除用法、`strings` 被魔改移除用法），`CalcOpenRouterCacheCreateTokens` 采纳上游 `QuotaRoundChecked` 饱和语义 |
| `relay/channel/ollama/adaptor.go` | 整个 Ollama 渠道已移除（`6745718c`） | 上游 `ba2e9287` 新增 Claude/OpenAI Responses passthrough | `668213d7`：2026-08-29 同步 6 提交；modify/delete 冲突维持删除——本地已无 Ollama 渠道，上游 passthrough 随之弃用 |
| `service/quota_saturation_test.go` | `PreConsumeBilling` 已短路（永不预扣），上游旧饱和测试在魔改线不适用 | 上游 `a073f74b` 新增 `TestCalcViolationFeeQuotaSaturates`、`TestCalcOpenRouterCacheCreateTokensDoesNotWrap` 及 `net/http`/`relaykit/types` import | `668213d7`：2026-08-29 同步 6 提交；保留上游 2 个仍适用的饱和测试与 import，弃用 2 个 `TestPreConsumeBilling*`（短路后永不返回错误）并同步清理孤儿 import `net/http`、`relaykit/types` |
