...
> 对应分支:`personal` 基线 `317e9ddd`(2026-08-29 刷新至 `949e1e69`；`personal` 线同步流程见「同步上游」节)
...
| `relay/channel/ollama/adaptor.go` | 整个 Ollama 渠道已移除（`6745718c`） | 上游 `ba2e9287` 新增 Claude/OpenAI Responses passthrough | `949e1e69`：2026-08-29 同步 6 提交；modify/delete 冲突维持删除——本地已无 Ollama 渠道，上游 passthrough 随之弃用 |
| `service/quota_saturation_test.go` | `PreConsumeBilling` 已短路（永不预扣），上游旧饱和测试在魔改线不适用 | 上游 `a073f74b` 新增 `TestCalcViolationFeeQuotaSaturates`、`TestCalcOpenRouterCacheCreateTokensDoesNotWrap` 及 `net/http`/`relaykit/types` import | `949e1e69`：2026-08-29 同步 6 提交；保留上游 2 个仍适用的饱和测试与 import，弃用 2 个 `TestPreConsumeBilling*`（短路后永不返回错误）并同步清理孤儿 import `net/http`、`relaykit/types` |
