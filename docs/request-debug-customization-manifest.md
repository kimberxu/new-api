...
- **共享库上的临时验证测试（硬约束）**：`.env` 的 `SQL_DSN` 指向共享 Supabase 测试库，本机可能直接用它跑实例。凡用该 DSN 做一次性验证的临时用例，清理**只能按夹具 id `DELETE` 行**；**禁止 `Migrator().DropTable()` 应用自身的表**（`channels`/`model_groups`/`model_group_items`/`channel_disabled_models` 等）——2026-09-16 已发生过一次：临时 PG 用例 DropTable 四张应用表致其从共享库消失，靠重跑 `AutoMigrate` 清单才还原（该库当时无渠道数据，未造成实际损失）。需要建表验证时改用**专用临时表名**并只删该表，参照 `model/token_migration_test.go`、`model/prefill_group_migration_test.go`。
...
| 成员级重试排除（同渠道兄弟成员接管） | `0c5ba81c1` | 中（`controller/relay.go`、`model/channel_cache.go`、`model/model_group_select.go`、`service/channel_select.go`） | `独立文件`（model/channel_exclude.go）+ `挂载点`（channel_cache.go/model_group_select.go/channel_select.go/relay.go） | 否 |
...
**2026-09-01 增强（模型组页降级标识）**：模型组页面成员行新增「降级中」徽章（`GET /api/channel/demoted` 5s 轮询，暂停于后台失焦），按 `(channelId, routableModelName)` 匹配成员（手动组 routable name 可能与上游模型不同），悬停显示降级来源(tps/ttft)与剩余恢复时长；组头部同步显示降级成员计数。仅展示，不影响路由行为。
...
- `web/src/features/model-groups/components/model-groups-page.tsx` - 5s 轮询 `/api/channel/demoted`，按 `(channelId, routableModelName)` 合并降级信息注入 `GroupCard`
- `web/src/features/model-groups/components/group-card.tsx` - 成员行降级徽章（来源 + 剩余时长 tooltip）+ 组头部降级计数徽章（按组名匹配降级记录）
- `web/src/features/model-groups/components/__tests__/group-card.test.tsx` - 降级徽章回归测试（头部计数 / 行徽章 / 组名与上游模型解耦匹配）
- `web/src/i18n/locales/*.json` - `Demoted ({{time}})` / `{{count}} demoted` 七语言文案
...
- [2026-08-31 同步 21 提交] 上游 task 插件系统（#7076）新增的计费契约测试与免费语义冲突，按「保留持久化/协议/映射断言、改计费断言」适配：`relay/relay_task_test.go` 5 处 `model_price_error` 断言改 `do_request_failed`（免费放行后请求真实发出）；`controller/relay_task_plugin_test.go`/`plugin_protocol_test.go`/`plugin_native_e2e_test.go` 4 处 `settle` 事件断言去 settle（`SettleBilling` 短路使 `Billing.Settle` 永不被调用），settle-failure 测试改写为验证「短路后任务照常 durable、无 refund」；弃用 8 个上游新增计费/OAuth UI 孤儿测试（pricing 6 + user-binding-dialog + usage-facts，对应组件已删）
...
## 错误日志记录实际上游模型
`processChannelError` 的错误日志 `other` 字段追加 `is_model_mapped` / `upstream_model_name`，使错误日志列表的 ModelBadge 能像成功日志一样通过 popover 显示请求模型与实际模型，错误日志详情页 Model Mapping 区段自动生效。
### 文件清单
**改动（最小插入）：**
- `controller/relay.go` - `processChannelError` 在 `other["admin_info"]` 之后、`AppendTaskPluginContextAuditInfo` 之前追加 `is_model_mapped`/`upstream_model_name` 字段（4 行）
**新增：**
- `controller/relay_error_log_test.go` - 测试：映射模型时写入、未映射时省略
### 验证
...
## 渠道测试复用 chat→responses 全局策略（测试与真实转发一致；2026-09-04 出站统一）

管理后台「测试」此前直调 `POST /v1/chat/completions`，绕过 `relay/compatible_handler.go:78` 的 `ShouldChatCompletionsUseResponsesGlobal` → `textRequestViaResponses` 转译链路；对仅支持 `/v1/responses` 的上游（如 `opencode.ai/zen` 的 `muse-spark-*`）真实用户流量正常、但渠道测试报 `500 Internal server error` / `EOF`。更深层的分叉：请求模型为可路由组名 `all-text-only` 时，真实链路经 `model_group → model_mapping` 映射到 `muse-spark-...` 才命中 `model_patterns=["muse-spark"]`，测试侧若以入站名直判则永远不命中，导致「真实已转 responses、探测仍走 chat」。

本次修复统一以**出站模型**（`UpstreamModelName`）判定，真实与探测同口径：

### 行为

- 真实转发：`relay/compatible_handler.go:78` 与 `relay/claude_handler.go:80` 已在 `ModelMappedHelper + ApplyReasoningModelSuffix` 之后，`UpstreamModelName` 已是最终出站名（含 `model_group` 合并、加权/链式 `model_mapping`、后缀裁剪）。门控改 `ShouldChatCompletionsUseResponsesGlobal(info.ChannelId, info.ChannelType, info.GetOutboundModelName())`，`GetOutboundModelName() = coalesce(UpstreamModelName, OriginModelName)`，保留 `PassThroughRequestEnabled / PassThroughBodyEnabled` 短路。
- 探测侧：`controller/channel-test.go:testChannel` 在 `endpoint_type == ""` 且未映射前即尊重策略切 `openai_response`，但判定输入为**出站候选集**而非入站名：`candidateOutboundModelsForTest` 对 `model.ApplyModelGroupMemberMapping(channel.GetModelMapping(), testModel, channel.Id)` 合并后的 JSON 做 BFS 展开（depth 8 / width 32，`string` 直连与 `[]any` 加权分支全枚举），任一终态出站命中策略即走 `/v1/responses`，覆盖加权随机分支；`normalizeTestOutboundModel` 复用 `ApplyReasoningModelSuffix` 裁剪推理后缀，**门控仅判剥离后**（与真实一致，`^gpt-5-high$` 类按后缀原文不误判）；非 chat 排除（`rerank`/`embedding`/`m3e`/`bge`/`embed`/`MokaAI`/`seedream`）按候选出站逐项过滤；`OpenRouter` 的 `-thinking` 剥离经 `ConvOptions().OpenRouterDialect` 判定，`testOutboundRelayInfo` 已补 `ChannelType/Id` 使测试与真实一致。`model.ResolveModelGroupUpstreamModel` 追加 `DB==nil` 守卫，避免无 DB 单测 panic。
...
- `relay/common/relay_info.go` - 新增 `GetUpstreamModelName()` / `GetOutboundModelName()`（出站统一入口，注释约束后续门控必用出站）
- `relay/compatible_handler.go:78` / `relay/claude_handler.go:80` - 门控改出站，保留透传短路与 `applySystemPromptIfNeeded` / `newApiErr` 原语义
- `relay/helper/reasoning_suffix.go` / `relay/helper/model_mapped.go` / `middleware/distributor.go:660` - 语义关联（映射与后缀的真实来源，不改动）
- `model/model_group_upstream.go` - `ResolveModelGroupUpstreamModel` 增 `DB==nil` 守卫；`ApplyModelGroupMemberMapping` 为探测枚举的合并入口
- `controller/channel-test.go` - `shouldForceResponsesForTest` 重写为出站语义（逐候选过滤 + 单判剥离后 + BFS 候选集）+ `normalizeTestOutboundModel` / `testOutboundRelayInfo`（补 ChannelType/Id）+ `candidateOutboundModelsForTest`（BFS 展开）+ `testChannel:290` 挂载点 + `413` 捕获三分支
- `controller/channel_captured_request.go` - 语义关联（捕获分支失配修复的消费方）
- 测试：`relay/common/relay_info_outbound_test.go`（出站回落语义）、`service/openai_chat_responses_outbound_test.go`（策略按出站判）、`controller/channel_probe_outbound_test.go`（组名映射、加权任一分支、推理后缀、透传不强制、`OpenRouter -thinking`、链式展开）
...
rebase 210 魔改提交全部重放，上游 `upstream/main` 完全包含于 HEAD。归位提交：`07c7b0ede`（后端）、`c4d40acef`（前端）、`a2cfd3209`（测试）、`794ee2ee2`（安全验证弹窗契约修复）、`da06b7e0a`（前端测试归位）。关键决策：

- **上游 6 认证加固提交（`d8cb17744`/`45c3fbe8a`/`3e84ec0ab`/`a8729b5c3`/`0973dc2b8`/`6f2333990`）**：Telegram OAuth 迁移统一 OAuth（`service/oauth/telegram.go` 及 oauth 包整体删除，`service/security_verification.go` 内联 `oAuthProviderInfo` 接口 + `oAuthProviders()` 空 stub 保留 /verify 路由与 2FA、channel.key.read scope）；`controller/login_verification.go` 剥离 LoginPasskeyBegin/Finish；`controller/audit.go` 对齐上游 `RecordOperationAuditLog` 新 9 参签名（operatorRole、*AuditAdminInfo、*AuditRequestInfo、c）并补 `tokenAuditParams`/`tokenBatchAuditParams`
- **上游 `0c76e4dae` 模型/厂商/定价重做（13343 行）被强制带入**，与定价删除对冲：前端删 model-pricing 面板、features/pricing、price-sync 残留；`service/security_verification.go` 的 oauth/passkey 枚举与 GetVerificationRequirements 分支剥离；metadata-editing/model-listing 测试删 pricing describe/用例
- **`routeTree.gen.ts` 取魔改旧表**（TanStack 生成文件不手拼，靠 `bun run build` router plugin 重生成，构建通过）；secure-verification/auth api/keys columns/profile 取上游版为底（上游调用方依赖上游导出）
- **同步后测试归位（`a2cfd3209`）**：`channel_test_request_test` 兼容性断言改形状断言（问句∈`testUserMessages` 池 + max tokens∈[16,64]，适配魔改 3e70f2f99 随机测活）；`model_management_test` 夹具补 `ModelGroup/ModelGroupItem/ModelGroupReference/ChannelDisabledModel` 建表（裸 AutoMigrate 清单不含模型组路由四表，`InitChannelCache→SyncAllModelGroups` 静默吞错致路由索引空）；`model/missing_models.go` 合并 ability 直配来源（模型组路由下渠道直配未入组模型仍判「系统引用但缺 metadata」，否则上游 sync preview 误标 catalog）；`model/main.go` 列名包变量预置反引号默认（手工 `gorm.Open` 测试未跑 `initCol` 时 `commonKeyCol` 空串致 `WHERE IN` 语法错）；删 security_account/security_enrollment/redemption_batch/enrollment 等已删功能深耦合测试
- **安全验证弹窗契约失配（`794ee2ee2`）**：上游 `6f2333990` 重构的 `SecureVerificationState` 为判别联合（`phase/input/requirements`），而本地 dialog 仍读旧接口 `state.code/title/loading`——任何安全验证弹窗（2FA/改密/删号/绑定/渠道 key 读取）一打开即 `state.code.trim()` TypeError 整树崩溃（enrollment 测试的 uncaught exception 暴露）。恢复上游版 dialog（274 行，passkey/password/oauth 分支仅依赖 `requirements` 返回值，不依赖已删 oauth 包），9 个 `{...dialogProps}` spread 调用点零改动；verification-flow 11 用例 + enrollment 跑绿
...
- 兼容性：18 个提交均带 `-x` 尾注；其中 **17 个 patch-id 与上游原提交一致**（`git rev-list --cherry-mark --right-only personal...upstream/main | grep -c '^='` 实测 17），**1 个例外** `c79b74b68` → `fc82ee85b`（手工 `git rm` 4 个已删文件致 patch 变更）。日后若做全量 rebase：17 个被默认丢弃不重复，`c79b74b68` 会被重新拣起、需手工再 `git rm` 一次（那 4 处 modify/delete：`user-binding-dialog.tsx` 及其测试、`web/src/routes/pricing/{index,$modelId/index}.tsx`）。
- **核对命令选型警告**：判断「某上游提交是否已纳入」**不要**用 `git rev-list --cherry-mark --right-only personal...upstream/main`——它按 patch 等价性分类，手工解冲突的 `c79b74b68` 会被列为 `+`（非等价侧），每轮都误报为「新提交」。应改用 `-x` 尾注检索：`git log --oneline --grep="cherry picked from commit <sha>" personal`（命中 `fc82ee85b`），不受手工解冲突影响。

### 探针与 merge 口径实测补充（2026-09-16 复核）

- **merge-tree 冲突数口径**：`--name-only` 输出首行是 tree hash，第 2–131 行才是冲突路径清单（本窗口 **130 行**，与描述行数 130 一致：36 内容 + 93 修改-删除 + 1 重命名；第 132 行空行）；第 133 行起的「自动合并 X」与「冲突（内容）：…」是逐文件日志，**不是**冲突清单，勿混入计数。实际 `git merge --no-commit` 落工作区 **125 文件**（92 DU + 32 UU + 1 UA）。
- **全量 merge 实测**：92 个 DU 全部为 personal 主动删除（92/92 在基线 `4fc9d1f1f` 存在），`git rm` 循环即清零，零人工判断成本。但 merge 会**静默带入 94 个新增文件**（其中 29 个属已删功能面：`model/legacy_dalle_pricing.go`、`pkg/billingexpr/fixed.go`、`relay/responses_websocket.go`、`pkg/wsmanager/`、`relay/request_billing.go`、`service/image_billing.go`、`model/passkey_option.go` 等），无冲突落线，需事后逐个 `git rm`。
- **32 个 UU 溯源**：11 个 i18n locale + 21 个代码。逐文件追 `git log <base>..upstream/main -- <file>`：**28 个仅由本线主动排除的提交引起**（`9fe0457ee`/`74629e29f`/`12be9975c`/`385d2dfd1` 等）、**3 个混杂**（`plugins/alibaba_responses_test.go`、`plugins/tasks/alibaba/plugin.js` 由纳入的 `a20574136` + 排除的 `74629e29f` 同改；`web/src/main.tsx` 由纳入的 `c79b74b68` + 排除的 `12be9975c` 同改）、**0 个仅由已纳入提交引起**。即：merge 的内容冲突几乎全部来自「已决定不要的功能」，为它们解冲突无收益。
