...
> 对应分支：`personal` 基线 `317e9ddd`（2026-09-09 刷新至 `a2cfd3209`；历史 deploy 功能部分可用 `git diff upstream/main...317e9ddd` 核对。`deploy` 分支已于 2026-09-04 删除，留档 tag `deploy-image-*` 仅历史回滚，分支拓扑见 `docs/local-github-workflow.md`）
...
rebase 210 魔改提交全部重放，上游 `upstream/main` 完全包含于 HEAD。归位提交：`07c7b0ede`（后端）、`c4d40acef`（前端）、`a2cfd3209`（测试）。关键决策：
