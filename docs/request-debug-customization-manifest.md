...
- 兼容性：18 个提交均带 `-x` 尾注；其中 **17 个 patch-id 与上游原提交一致**（`git rev-list --cherry-mark --right-only personal...upstream/main | grep -c '^='` 实测 17），**1 个例外** `c79b74b68` → `fc82ee85b`（手工 `git rm` 4 个已删文件致 patch 变更）。日后若做全量 rebase：17 个被默认丢弃不重复，`c79b74b68` 会被重新拣起、需手工再 `git rm` 一次。

### 探针与 merge 口径实测补充（2026-09-16 复核）

- **merge-tree 冲突数口径**：`--name-only` 输出首行是 tree hash，第 2 行起才是冲突路径清单（本窗口 135 行）；其后的「自动合并 X」与「冲突（内容）：…」是逐文件日志，**不是**冲突清单，勿混入计数。真实冲突按描述行取 **130 处**；实际 `git merge --no-commit` 落工作区 **125 文件**（92 DU + 32 UU + 1 UA）。
