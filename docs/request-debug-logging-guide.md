...
控制每一侧请求体快照的 `body` 字段最多保存多少字节：

```env
REQUEST_DEBUG_MAX_BODY_BYTES=32768
```

默认值为 `32768`，即 32 KiB。设置为 `0`、负数或无法解析的值时会恢复默认值。该配置不限制真实请求体大小，也不影响请求转发，只限制写入管理员调试日志的快照长度。

快照会先做语义化脱敏和正文压缩，再应用这个整体字节上限：

- 密钥类字段会替换为 `[REDACTED]`；
- `content`、`input`、`instructions`、`prompt`、`suffix`、`system`、`text` 等正文类字段会替换为长度摘要，例如 `[TRUNCATED text 280 bytes]`；
- 其他长字符串仍会保留前缀并标记截断；
- 最后如果整体 `body` 仍超过 `REQUEST_DEBUG_MAX_BODY_BYTES`，才会按字节数整体截断。

超过限制或发生字段级压缩时，日志仍会保存：
...
| `truncated` | 保存内容是否经过字段级压缩或整体大小限制截断 |
| `body` | 脱敏、正文摘要、字符串限制和整体截断后的内容；后端按字符串保存 |
...
请求体 `body` 如果是合法 JSON，Web 详情弹窗会自动用缩进格式展示，便于阅读嵌套参数和消息数组。该格式化只影响界面显示，不修改日志中的原始 `body` 字符串；复制按钮仍复制后端保存的原始内容。非 JSON 内容会保持原样显示。
...
2. 不要直接在生产环境无限提高存储上限；
3. 如确需查看更多非正文参数上下文，可短时间提高 `REQUEST_DEBUG_MAX_BODY_BYTES`；
4. 提示词正文类字段会被替换为长度摘要，提高 `REQUEST_DEBUG_MAX_BODY_BYTES` 不会恢复这些正文内容；
5. 排障后恢复为 `32768` 或更低值。
