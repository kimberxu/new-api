...
- **MySQL 不再作为验证目标（2026-09-16 项目所有者指示）**：`personal` 线实际部署只用 PostgreSQL，故 DB 相关验证只需覆盖 **SQLite + PostgreSQL** 两库；MySQL 实例不可得时**不构成阻塞项**，无需再在交付说明里登记「MySQL 未测」。仍应避免无谓破坏其他方言的写法（纯新增 SQL 尽量用跨库标准语法，成本为零时不做 MySQL 不兼容的改动），但不再为 MySQL 专门起实例验证。
