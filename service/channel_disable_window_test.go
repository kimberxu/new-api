...
func TestCheckAndRecordDisable_ConfiguredThreshold(t *testing.T) {
	...
	assert.False(t, CheckAndRecordDisable(1, 500, true), "first configured error should not trigger disable")
	// 2nd error: should trigger.
	assert.True(t, CheckAndRecordDisable(1, 500, true), "second configured error should trigger disable")
}
...
func TestCheckAndRecordDisable_UnconfiguredThreshold(t *testing.T) {
	...
	assert.False(t, CheckAndRecordDisable(1, 503, false), "1st unconfigured error should not trigger")
	assert.False(t, CheckAndRecordDisable(1, 503, false), "2nd unconfigured error should not trigger")
	assert.True(t, CheckAndRecordDisable(1, 503, false), "3rd unconfigured error should trigger")
}
...
func TestCheckAndRecordDisable_DifferentStatusCodesIndependent(t *testing.T) {
	...
	assert.False(t, CheckAndRecordDisable(1, 500, true))
	// Channel 1, status 502 — different key, should start fresh.
	assert.False(t, CheckAndRecordDisable(1, 502, true))
	// Channel 1, status 500 — 2nd error, should trigger.
	assert.True(t, CheckAndRecordDisable(1, 500, true))
}
...
func TestCheckAndRecordDisable_DifferentChannelsIndependent(t *testing.T) {
	...
	assert.False(t, CheckAndRecordDisable(2, 500, true))
	// Channel 1 — 2nd error, should trigger.
	assert.True(t, CheckAndRecordDisable(1, 500, true))
	// Channel 2 — 2nd error, should also trigger.
	assert.True(t, CheckAndRecordDisable(2, 500, true))
}
...
func TestCheckAndRecordDisable_ThresholdOne(t *testing.T) {
	...
	assert.True(t, CheckAndRecordDisable(1, 500, true), "threshold=1 should trigger on first error")
}
...
func TestCheckAndRecordDisable_ThresholdZero(t *testing.T) {
	...
	assert.False(t, CheckAndRecordDisable(1, 500, true), "threshold=0 should never trigger disable")
}
...
func TestCheckAndRecordDisable_ConfiguredAndUnconfiguredIndependent(t *testing.T) {
	...
	assert.False(t, CheckAndRecordDisable(1, 500, false))
...
func TestShouldDisableChannelWithDecision_ConfiguredError(t *testing.T) {
	...
	// Second call: should disable with a reason.
	decision = ShouldDisableChannelWithDecision(10, err)
