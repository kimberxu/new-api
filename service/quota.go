...
func PostAudioConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
	...
	gopool.Go(func() {
		...
		RecordFromRelayInfo(relayInfo, int64(usage.CompletionTokens), int64(usage.PromptTokens))
	})
}
...
func PostConsumeQuota(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) error {
	// [personal] 计费功能级移除：永不扣用户/令牌额度、永不发邮件。
	// 日志记录由调用方独立完成，不受影响。
	return nil
}
...
func postConsumeQuotaWithResult(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (result postConsumeQuotaResult, err error) {
	// [personal] 计费功能级移除：任何来源的额度都不扣减、不通知。
	// FundingApplied stays false so task callers see a free request.
	return result, nil
}
