...
func modelManagementDB(t *testing.T, kind, dsn string) *gorm.DB {
	...
	require.NoError(t, database.AutoMigrate(&model.Model{}, &model.Vendor{}, &model.Channel{}, &model.Ability{}, &model.Option{}, &model.User{}, &model.AuditLog{}, &model.ModelGroup{}, &model.ModelGroupItem{}, &model.ModelGroupReference{}, &model.ChannelDisabledModel{}))
		...
		t.Run(dialect.kind, func(t *testing.T) {
			...
			for _, rule := range []int{model.NameRuleExact, model.NameRulePrefix, model.NameRuleContains, model.NameRuleSuffix} {
				t.Run(fmt.Sprintf("rule_%d_exact_names_only_atomic_and_cached", rule), func(t *testing.T) {
					...
					cached, _, err := model.GetRandomSatisfiedChannel("default", name, 0, nil, nil)
					require.NoError(t, err)
					...
					for _, group := range []string{"default", "vip", "last-model-group"} {
						cached, _, _ = model.GetRandomSatisfiedChannel(group, name, 0, nil, nil)
						assert.Nil(t, cached)
					}
					cached, _, err = model.GetRandomSatisfiedChannel("default", name+"-keep", 0, nil, nil)
					require.NoError(t, err)
					require.NotNil(t, cached)
